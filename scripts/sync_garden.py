...
PARTNER='https://raw.githubusercontent.com/Sri014/Garden/main/working.m3u'
W=Path('playlist_working.m3u'); N=Path('playlist_nonworking.m3u'); R=Path('cross_sync.json')
def norm(s):
...
W.write_text(render(w),encoding='utf-8'); N.write_text(render(keep),encoding='utf-8'); R.write_text(json.dumps({'source':PARTNER,'repaired':len(repaired),'channels':repaired},indent=2,ensure_ascii=False),encoding='utf-8'); print(f'3502 cross-sync: repaired {len(repaired)} channels from Garden')
